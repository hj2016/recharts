...
echartR<-function(data, x=NULL, y, z=NULL, series=NULL, weight=NULL,
  ...
  for (t in 1:ifelse(is.null(z),1,length(timeslice)))  {
    ...
    for (i in 1:ifelse(is.null(xAxis1),1,2)){  # xAxises
      ...
      if(!is.null(varXAxis[['show']])){
        if(as.character(varXAxis[['show']])=="F"||as.character(varXAxis[['show']])=="FALSE"){
          tmpXAxis[['axisLabel']][['show']] <- varXAxis[['show']]
        }
      }
    ...
    for (i in 1:ifelse(is.null(yAxis1),1,2)){ # yAxises
      ...
      if(!is.null(varYAxis[['show']])){
        if(as.character(varYAxis[['show']])=="F"||as.character(varYAxis[['show']])=="FALSE"){
          tmpYAxis[['axisLabel']][['show']] <- varYAxis[['show']]
        }
      }
